...
//   forkDir '-' = removal mode (TCF-001): load only fleet-core + pi-subagents
//   and assert the pi-subagents tools/commands still register, fleet stays
//   intact, and NO trellis_subagent tool remains (no leftover from a removed
//   fork).
...
// ── 1. Load the extensions with the recording fake pi ──────────────────
let forkMod = null;
if (withFork) {
  forkMod = await jiti.import(`${forkDir}/index.ts`);
  forkMod.default(pi);
}
...
if (withFork && trellisSubagents !== 1)
  fail(`expected exactly one trellis_subagent tool, got ${trellisSubagents} (${toolNames.join(",")})`);
if (!withFork && trellisSubagents !== 0)
  fail(`expected NO trellis_subagent tool after fork removal, got ${trellisSubagents} (${toolNames.join(",")})`);
ok(withFork
  ? `exactly one trellis_subagent tool (all tools: ${toolNames.join(", ")})`
  : `no trellis_subagent tool after fork removal (all tools: ${toolNames.join(", ")})`);
...
ok(withFork ? "before_agent_start handler present" : "(no fork: before_agent_start not asserted)");
...
// TCF-001 (removal mode): pi-subagents' own commands must still register
// after the fork is removed; fleet must stay exactly one.
const subagentCommands = commands.filter((c) => c === "subagents:settings" || c === "subagents:sessions");
if (subagentCommands.length !== 2)
  fail(`expected subagents:settings + subagents:sessions after fork removal, got ${subagentCommands.join(",")} (${commands.join(",")})`);
ok(`pi-subagents commands present (${subagentCommands.join(", ")})`);

// ── 2. Two-tier agent resolution (global + project, project wins) ───────
//    (fork-only; skipped in removal mode)
if (withFork) {
...
console.log(
  withFork
    ? "composed-load: all assertions passed"
    : "composed-load: removal-mode assertions passed",
);
